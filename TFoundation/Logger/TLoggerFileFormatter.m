...
#define kTLoggerFormatStringWhenLogMsgIsNotJson @"{\"loglevel\":\"%@\",\"timestamp\":\"%@\",\"file\":\"%@\",\"fileandlinenumber\":\"%@:%lu\",\"jsonerror\":\"JSON Output Error when trying to create Loggly JSON\",\"rawlogmessage\":\"%@\"}"
...
@implementation TLoggerFileFormatter{
    id<TLoggerFieldsDelegate> logglyFieldsDelegate;
}

- (instancetype)init {
    if((self = [super init])) {
        // Use standard LogglyFields Delegate
        logglyFieldsDelegate = [[TLoggerFields alloc] init];
...
- (NSString *)formatLogMessage:(DDLogMessage *)logMessage {
    ...
    NSMutableDictionary *logfields = [NSMutableDictionary dictionaryWithDictionary:[logglyFieldsDelegate loggerFieldsToIncludeInEveryLogStatement]];
    ...
    if (outputJsonError) {
        return [NSString stringWithFormat:kTLoggerFormatStringWhenLogMsgIsNotJson, logLevel, iso8601DateString, filestring, filestring, (unsigned long)logMessage->_line, logMsg];
    }
